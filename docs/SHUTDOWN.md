...
[DESIGN-event-driven-dispatch.md](DESIGN-event-driven-dispatch.md)) and the AV
stopped reproducing.
...
defect.** No `_session_end` is written because Altium tears the script engine
down without granting a final tick. Detach writes it every time
(`reason=stop-requested`), so the finaliser itself works. Nothing leaks: zero
