...
**773 tools** across 21 categories.
...
| `pcb_get_object_classes` | readonly | live_only | Every class kind with resolved membership (nets, components, differential pairs). Membership is probed via `IsMember`, not declared, because `MemberCount`/`MemberName[]` are unavailable and the non-net kind constants are undeclared in DelphiScript. |
